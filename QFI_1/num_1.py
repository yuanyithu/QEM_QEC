...
def haar_random_unitary(n, k):
    """
    生成Haar随机的2**k维子空间在2**n维空间中的投影矩阵
    ...
    # 生成随机高斯矩阵
    Z = np.random.randn(N, M) + 1j * np.random.randn(N, M)
    
    # 使用Householder反射的改进QR分解
    Q = np.zeros((N, M), dtype=complex)
    
    for j in range(M):
        # 生成随机向量
        x = np.random.randn(N-j) + 1j * np.random.randn(N-j)
        norm_x = np.linalg.norm(x)
        x = x / norm_x
        
        # Householder反射
        v = x.copy()
        v[0] += np.exp(1j * np.angle(x[0]))  # 确保数值稳定性
...
def haar_random_wishart(n, k):
    """使用Wishart矩阵的特征向量，适合生成高纠缠态"""
    N = 2**n
    M = 2**k
...
def sample(n,k,p,N_til):
    # Q = generate_random_orthonormal_vectors(n, k)
    # Q = haar_random_unitary(n,k)
    Q = haar_random_wishart(n,k)
...
def generate(n,k,p,N):
    print("processing p = ",p," ,  n = ",n)
    D = noise_super_operator(p)
    N_til = np.einsum("ijkl->ikjl",tensor_power(D,n)).reshape([4**n,4**n])
    result = []
    for i in range(N):
        complete = int(i*50/N)
        uncomplete = 50-complete
        bar = '['+'*'*complete+'-'*uncomplete+']'
        output_str = '\r'+bar
        sys.stdout.write(output_str)
        sys.stdout.flush()
        result.append(sample(n,k,p,N_til))
    print()
    return result
...
def maxs_from_samples(samples_by_n, n_list):
    """按 n_list 顺序给出最大值列表。"""
    return [np.min(samples_by_n[n]) for n in n_list]
...
def plot_sweep(n_list, p_list, k, N, gen_fn, ylim=(0, 4), jitter=0.0):
